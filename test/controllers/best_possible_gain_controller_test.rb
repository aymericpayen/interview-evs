...
class BestPossibleGainControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should return 200 and max_gain = 0 when prices does not fluctuate' do
  ...
  test 'should return 200 and max_gain = 0 when prices is only decreasing during the day' do
    PotatoPrice.create!(time: DateTime.new(2022, 8, 22, 9, 0, 0).strftime("%FT%T.%LZ"), value: 100.25)
